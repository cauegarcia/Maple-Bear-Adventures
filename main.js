...
window.addEventListener('scroll', () => {
  ...
  if (distanceElement.top < window.innerHeight - 150) {
    ...
    setFade.fadeRight[index].style.opacity = '1';
    setFade.fadeRight[index].style.transform = 'translateX(0)';
  };
  if (distanceElement.top < 0 - 50 || distanceElement.top > window.innerHeight) {
...
const carousel = (() => {
  ...
  leftBtn.addEventListener('click', (e) => {goLeft()}, {once: true});
  rightBtn.addEventListener('click', (e) => {goRight()}, {once: true});
  cards.forEach(card => {
    card.addEventListener('click', (e) => {changeCardPlace(e)});
  })

  const states = ['activity1', 'activity2', 'activity3', 'activity4', 'activity5'];
  const positions = {
    0: {x: '-50%', z: '0', o: '1'},
  ...
  const goRight = () => {
    ...
    render();
    setTimeout(() => {rightBtn.addEventListener('click', goRight, {once: true})}, 1000);
  }
  const goLeft = () => {
    const tempState = [];
    states.forEach((position, index) => {
    ...
    render();
    setTimeout(() => {leftBtn.addEventListener('click', goLeft, {once: true})}, 1000);
  }
  const render = () => {
...
const setGalleryEffect = (() => {
  ...
  const translate = (div) => {
    ...
    setTimeout(() => {div.addEventListener('mouseenter', translate, {once: true})}, 2000);
  }
  divs.forEach(div => div.addEventListener('mouseenter', (div) => {return translate(div)}, {once: true} ));
